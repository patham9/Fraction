...
#define _select(Type,x,in,M,having,n,elements,as,z,if,c,ass,N,delivering,m,elementss) {Type Selector(Type x){return z;}int Condition(Type x){return c;} int *ConditionSave=(int*) calloc(n,sizeof(int)); int i,cnt=0; for(i=0;i<n;i++){ConditionSave[i]=Condition(M[i]);if(ConditionSave[i]){cnt++;}} N=(Type*) malloc(cnt*sizeof(Type)); m=cnt; cnt=0; for(i=0;i<n;i++){if(ConditionSave[i]){N[cnt]=Selector(M[i]);cnt++;}} free(ConditionSave);}
#pragma endregion
//-------------------------------------------//
...
float op_max(float a,float b);
float op_min(float a,float b);
//::::::::::::::::::::::::::::::::::::::::::://
#endif
/*................COMMENTS...................//
>>todo:
