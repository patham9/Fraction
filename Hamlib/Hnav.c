...
float hnav_WorldToMouseCoordX(float x)
{
	return hrend.width*(hnav.zoom*x+hnav.difx+0.5);
}
float hnav_WorldToMouseCoordY(float y)
{
	return hrend.height*(1-hnav.zoom*y+(hnav.dify+0.5));
}
float hnav_MouseToWorldCoordX(float x)
{
	return 1/hnav.zoom*(((float)x)/hrend.width-(hnav.difx+0.5f));
}
float hnav_MouseToWorldCoordY(float y)
{
